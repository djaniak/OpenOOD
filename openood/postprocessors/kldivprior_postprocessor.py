...
class PriorKlDivPostprocessor(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        if self.use_augumented:
            ...
            conf = self.kl_div(net, z1, mu1, logvar1) + self.kl_div(
                net, z2, mu2, logvar2
            )
        else:
            conf = self.kl_div(net, z3, mu3, logvar3)
    ...
    def kl_div(
        ...
        **kwargs: Any
        ...
        return -(
            net.model.prior.log_prob(x.unsqueeze(0))
            - log_normal_diag(x.unsqueeze(0), mu, logvar, reduction="avg", dim=0)
        ).sum(-1)
